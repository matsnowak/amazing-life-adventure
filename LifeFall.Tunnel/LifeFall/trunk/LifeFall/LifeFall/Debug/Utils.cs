    ...
    public static class Utils
    {
        ...
        public static Model LoadModelWithBoundingSphere(ref Matrix[] modelTransforms, string asset, ContentManager content)
        {
            ...
            foreach (ModelMesh mesh in newModel.Meshes)
            {
                ...
                BoundingSphere transMeshSphere = Utils.TransformBoundingSphere(origMeshSphere, modelTransforms[mesh.ParentBone.Index]);
        ...
        private static Matrix[] AutoScale(ref Model model, float requestedSize)
        {
            ...
            Matrix boundingSphereTransform = Matrix.CreateScale(scalingFactor);
            BoundingSphere newbSphere = TransformBoundingSphere(bSphere, boundingSphereTransform);

            model.Tag = newbSphere;
            return modelTransforms;
        }

        public  static Model LoadModel(string asset)
        {
            Matrix[] modelTransforms = new Matrix[0];
            Model m = LoadModelWithBoundingSphere(ref modelTransforms, asset, Costam.Game.Content);

            return m;
        }

        public static Model LoadModel(string asset, float size)
        {
            // TODO: Można scalić to z LoadModelWithBoundingSphere i z AutoScale, bo teraz sa wywoływane 3 razy kopiowania
            Model m = LoadModel(asset);
            AutoScale(ref m, size);
            return m;
        }
