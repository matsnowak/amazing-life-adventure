...
namespace LifeFall.Models
{
    public class TunnelPath
    {
        ...
        private int InitLength
        {
            get
            {
                return 11; // uzależnić od rozdielczości jakoś i odległości jaką musimy widzieć itp....
            }
        }
        ...
        private void DrawLine(GraphicsDevice graphicsDevice, List<Vector3> vectorsList)
        {
            if (vectorsList.Count > 0)
            {
                ...
                {
                    if (i % 2 == 0)
                        vectors[i] = new VertexPositionColor(vectorsList.ElementAt(i), Color.Red);
                    else
                        vectors[i] = new VertexPositionColor(vectorsList.ElementAt(i), Color.Gold);
                }
                graphicsDevice.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.LineStrip, vectors, 0, vectors.Count() - 1);
            }
        }
        ...
        public List<Vector2> PathPoints2D
        {
            get
            {
                ...
                {
                    pathPoints2D.Add(new Vector2(point.X, point.Y));
                }
        ...
        public void DrawTunnelPath2D(GraphicsDevice graphicsDevice)
        {
            //if (PathPoints2D.Count > 0)
            //{
            //    VertexPositionColor[] vectors = new VertexPositionColor[PathPoints2D.Count];

            //    for (int i = 0; i < PathPoints2D.Count; i++)
            //    {
            //        if (i % 2 == 0)
            //            vectors[i] = new VertexPositionColor(PathPoints2D.ElementAt(i), Color.Red);
            //        else
            //            vectors[i] = new VertexPositionColor(PathPoints2D.ElementAt(i), Color.Gold);
            //    }
            //    graphicsDevice.DrawUserPrimitives<VertexPositionColor>(PrimitiveType.LineStrip, vectors, 0, vectors.Count() - 1);
            //}
        }
